...
export class ContentEngine {
  ...
  async refreshContent(options?: ContentEngineOptions): Promise<ContentItem[]> {
    ...
    try {
      ...
      if (typeof chrome !== "undefined" && chrome.tabs) {
        ...
        if (tab?.id) {
          try {
            content = await ChromeExtensionApi.sendMessage(
              "getRandomContent",
              {
  ...
  // 直接获取内容（不通过内容脚本）
  private async getContentDirectly(
    options: Required<ContentEngineOptions>,
  ): Promise<ContentItem[]> {
    // 这个方法主要用于后台脚本或测试
        ...
        if (
          ...
          for (const key of possibleKeys) {
            try {
              ...
              console.log(
                `[ContentEngine] 缓存项 ${key}:`,
                cacheItem[key] ? "存在" : "不存在",
              );
              ...
              if (
                ...
                console.log(
                  `[ContentEngine] 从 ${key} 获取到 ${content.length} 个内容`,
                );
        ...
        if (typeof chrome !== "undefined" && chrome.tabs) {
          ...
          if (tab?.id) {
            try {
              ...
              users = await ChromeExtensionApi.sendMessage(
                "getFollowedUsers",
                {
                  platform,
                },
  ...
  // 获取缓存的关注用户
  private async getCachedFollowedUsers(
    platform?: Platform,
  ...
  // 从所有平台获取缓存的关注用户
  private async getCachedFollowedUsersFromAllPlatforms(): Promise<
    FollowedUser[]
  > {
        ...
        for (const key of followingsKeys) {
          try {
            ...
            if (
              ...
              console.log(
                `[ContentEngine] 从 ${key} 获取到 ${users.length} 个用户`,
              );
              allUsers.push(...users);
            }
          } catch (error) {
  ...
  getStatus(): {
    ...
    const now = Date.now();
    const uptime =
      this.isRunning && this.lastRefresh > 0 ? now - this.lastRefresh : 0;
  ...
  // 删除指定用户及其相关数据
  async deleteUser(userId: string, platform?: Platform): Promise<void> {
    try {
      this.log(`开始删除用户 ${userId}...`, "info");
      ...
      for (const key of userCacheKeys) {
        if (key.includes('_videos_') && key.includes(userId)) {
          try {
            ...
            console.error(`[ContentEngine] 计算缓存大小时出错:`, error);
          }
        }
      }
