# Chrome Extension 打包指南

## 📦 打包命令

### 基础打包
```bash
bun run package
```

### 开发环境打包
```bash
bun run package:dev
```

### 完整发布流程（清理+打包）
```bash
bun run release
```

## 🚀 打包流程

脚本会自动执行以下步骤：

1. **清理旧包** - 删除 `release/` 目录中的旧版本
2. **构建项目** - 运行 `scripts/build.ts` 构建 dist 目录
3. **验证文件** - 检查必要的扩展文件是否存在
4. **创建版本信息** - 生成 `version.json` 包含构建信息
5. **生成 ZIP 包** - 创建适用于 Chrome Web Store 的 zip 文件
6. **生成报告** - 输出详细的打包信息

## 📁 输出文件

打包完成后，你会在 `release/` 目录中找到：

```
release/
├── onlyfollow-v1.0.0.zip          # Chrome Web Store 上传文件
├── package-report-v1.0.0.json    # 详细打包报告
└── ...
```

## 📊 打包报告

报告包含以下信息：

- **packageName**: 扩展名称
- **version**: 版本号
- **packagePath**: 包文件路径
- **packageSize**: 包文件大小
- **buildTime**: 构建时间
- **filesIncluded**: 包含的文件数量

## ⚠️ 注意事项

### 系统要求
- 需要安装 `zip` 命令行工具（大多数系统已预装）
- macOS/Linux: 通常已预装
- Windows: 可通过 Git Bash 或 WSL 使用

### 版本管理
- 版本号自动从 `package.json` 读取
- 每次发布前请确保版本号已更新

### 文件验证
脚本会检查以下必要文件：
- `manifest.json`
- `content.js`
- `background.js`
- `popup.html`

## 🌐 Chrome Web Store 上传

1. **获取包文件**
   ```bash
   ls release/*.zip
   ```

2. **上传到开发者控制台**
   - 访问 [Chrome Web Store Developer Dashboard](https://chrome.google.com/webstore/developer/dashboard)
   - 上传生成的 ZIP 文件

3. **填写商店信息**
   - 插件名称、描述
   - 截图和图标
   - 隐私政策链接（如需要）

## 🔧 自定义配置

可以在 `scripts/package.ts` 中修改配置：

```typescript
const config: PackageConfig = {
  outputDir: 'release',      // 输出目录
  distDir: 'dist',          // 构建目录
  packageName: 'onlyfollow', // 包名前缀
  version: '1.0.0'          // 版本号
};
```

## 🐛 故障排除

### zip 命令未找到
```bash
# macOS
brew install zip

# Ubuntu/Debian
sudo apt-get install zip

# CentOS/RHEL
sudo yum install zip
```

### 权限问题
```bash
chmod +x scripts/package.ts
```

### 构建失败
```bash
# 手动构建检查
bun run build
ls dist/
```

## 📋 上传前检查清单

- [ ] 版本号已更新
- [ ] `manifest.json` 信息正确
- [ ] 所有图标文件存在且尺寸正确
- [ ] 插件在本地测试正常
- [ ] 隐私政策页面已创建（如需访问用户数据）
- [ ] 商店截图已准备
- [ ] ZIP 包大小合理（建议 < 50MB）

## 🎯 快速发布

```bash
# 一键发布流程
bun run release

# 检查生成的包
ls -lh release/*.zip
```

打包完成后，直接将 ZIP 文件上传到 Chrome Web Store 即可！
